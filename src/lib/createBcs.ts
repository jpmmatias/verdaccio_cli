...
export const createBcs = async (bcs: string[]) => {
  log.message('Criando BCs...')
  const createdBcsPromisses: Promise<void>[] = []
  new Promise((resolve, reject) => {
    bcs.forEach(bc => {
      const cliCommand = `yarn workspace ${bc} rollup`
      const createPromise = new Promise<void>((innerResolve, innerReject) => {
...
export const publishingBcs = async (bcs: string[]) => {
  log.message('Publicando BCs no Verdaccio...')

  const publishingBcsPromisses: Promise<void>[] = []
  new Promise((resolve, reject) => {
    bcs.forEach(bc => {
      ...
      const createPromise = new Promise<void>((innerResolve, innerReject) => {
        exec(cliCommand, (error, stdout, stderr) => {
          if (error) {
            ...
            if (responseCode === 409) {
              log.error(
                `Pacote ${bc} ja existe no Verdaccio, delete o pactote antes de criar novamente ou faça o update do package`,
              )
              exit()
            }
            innerReject(error)
